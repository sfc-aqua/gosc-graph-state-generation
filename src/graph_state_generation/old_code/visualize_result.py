...
adj_list_gen = gen_erdos_renyi_graph_single_component(
    n_nodes_ran, n_edges_ran
)  # generate random graph for testing

if adj_list_gen != None:
    # run compilation
    num_step_scheduled, num_stablizers, adj_list_gen, stablizer_on_qubits, board_step = twotile.main(
        adj_list_gen, n_nodes_ran
    )
        ...
        for j in range(0, col):
            if isinstance(row_board[j], numbers.Number):
                ...
                if qubit_id in operation_step.keys():
                    ...
                    if operation == ["Z"]:
                        rec = Rectangle(
                            (j, row - i - 1), width=1, height=1, facecolor="r", edgecolor="gray"
                        )
                    if operation == ["X"]:
                        rec = Rectangle(
                            (j, row - i - 1), width=1, height=1, facecolor="y", edgecolor="gray"
                        )
                    plt.text(
                        j + 0.5,
                        row - i - 1 + 0.5,
                        (row_board[j], operation),
                        fontweight="bold",
                        fontsize=7.5,
                    ...
                    rec = Rectangle(
                        (j, row - i - 1), width=1, height=1, facecolor="b", edgecolor="gray"
                    )
                    plt.text(
                        j + 0.5,
                        row - i - 1 + 0.5,
            ...
            if row_board[j] == "anc":
                # for ancilla patch
                rec = Rectangle(
                    (j, row - i - 1), width=1, height=1, facecolor="gray", edgecolor="gray"
                )
                # print(rec)
                ax.add_patch(rec)
